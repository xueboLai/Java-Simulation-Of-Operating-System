...
/**
 * This is a scheduling algorithm for operating system: Highest Penalty Ratio Next. It is a non-preemptive 
 * multi-programming algorithm. Processes' running priority will be determined by (T/t). T is the total time they have been in system, t is the running time.
 * The higher that number is, the more likely they are going to be run the next. It aims to provide scheduling 
...
public class HPRN {
	...
	public HPRN(ArrayList<Process> raw,ArrayList<Integer> random,String verbose){
		this.inputArr = raw;
	...
	public void process() throws IOException{
		System.out.println("Highest Penalty Ration Next:\n");
		...
		FileWriter fw = null;
		//fw = new FileWriter("testing_hprn.txt");
		...
		do{
			...
			Collections.sort(ready,new CompareFilesByHPRN());
			
			
			/*for(int counting = 0;counting<init.size();counting++){
				System.out.println("process "+counting+": totalTime: "+init.get(counting).getTotalTime()+"; running time: "+init.get(counting).getRunningTime()+"; Arrval: "+init.get(counting).getA()+"; counting in process: "+init.get(counting).getCount());
			}
			System.out.println("testing comparefile: "+new CompareFilesByHPRN().compare(init.get(1), init.get(0)));
			*/
...
//change
			...
			if(running.size()!=0){
				if(running.get(0).getC()==0){
					...
					//cpuUti++;
					running.get(0).incrementRunningTime();
			...
			if(running.size()==0){
				//System.out.println("The ready size is "+ready.size());
				if(ready.size()!=0){
					/*if(ready.get(0).getA()==counter&&ready.size()==2){
						running.add(ready.get(1));
						ready.remove(1);
						running.get(0).incrementRunningTime();
			...
			for(int b = 0;b<init.size();b++){
				if(init.get(b).getA()<this.counter){
					init.get(b).incrementTotalTime();
					//System.out.println("!!!total time: "+init.get(b).getTotalTime());
				}
			}
